...
use axum::{body::Body, extract::Request, http::{header::CONTENT_TYPE, StatusCode}};
use common::{
    mock_worker::{HealthStatus, MockWorkerConfig, WorkerType, set_slow_stream_chunks},
    AppTestContext,
};
use serde_json::json;
...
async fn queued_requests_do_not_hold_inflight_permit() {
    ...
    let resp2 = tokio::spawn(async move {
        app2.oneshot(stream_request()).await.unwrap()
    });
    ...
    assert_ne!(resp2.status(), StatusCode::TOO_MANY_REQUESTS, "req2 must enter the queue, not 429 at the req/s gate");
    drain(resp2).await;
    ctx.shutdown().await;
}
