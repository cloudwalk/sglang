...
use smg::config::builder::RouterConfigBuilder;
...
async fn inflight_cap_field_defaults_to_zero_and_is_settable() {
    ...
    assert_eq!(cfg.max_inflight_requests, -1, "-1 must be accepted (explicit disable)");
}
...
async fn inflight_cap_field_serde_roundtrips() {
    ...
    let from_stripped: RouterConfig = serde_json::from_value(stripped).expect("deserialize");    assert_eq!(
